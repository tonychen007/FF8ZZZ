...
int LZS::match_length=0;//of longest match. These are set by the InsertNode() procedure.
int LZS::match_position=0;
int LZS::lson[4097];//left & right children & parents -- These constitute binary search trees.
int LZS::rson[4353];
int LZS::dad[4097];
...
const QByteArray &LZS::decompress(const char *data, int fileSize, int max)
{
	...
	uint16_t curBuff=4078, adresse, premOctet=0, i, length;
	const uint8_t *fileData = (const uint8_t *)data;
	const uint8_t *endFileData = fileData + fileSize;

	// Impossible case
	if(uint64_t(sizeAlloc) > 2000 * uint64_t(fileSize)) {
		qWarning() << "LZS::decompress impossible ratio case" << sizeAlloc << 2000 * uint64_t(fileSize);
		result.clear();
		return result;
	}
	...
	forever
	{
		...
		{
//			uint16_t twoBytes = *((const uint16_t *)fileData);
...
void LZS::DeleteNode(int p)//deletes node p from tree
{
	int q;
